...
class FCNet(nn.Module):
    def __init__(self, in_len=4000, num_classes=2, hidden_1=64, hidden_2=128):
        super().__init__()
        self.n_class = num_classes
        self.fc1 = nn.Linear(in_len,hidden_1)
...
# This is the residual network created by Eddie
class PSDNResNet(nn.Module):
...
class SpectroscopyTransformerEncoder(nn.Module): 
  def __init__(self,
               ...
               num_classes=2,
               pos_fix=False): # generic number of classes (this can be adjusted)
    super().__init__()
    ...
    if pos_fix:
        self.positional_embedding = torch.linspace(0, 1, num_patches+1).unsqueeze(0).unsqueeze(-1).repeat(1, 1, embedding_dim)
    else:
...
class InceptionNetwork_PreT(nn.Module):
    def __init__(self, num_classes): # generic number of classes (this can be adjusted)
        super().__init__()
        self.pre = PreBlock()
...
if __name__=="__main__":
    ...
    FC_input_data = torch.randn(32, 4000).to(device)
