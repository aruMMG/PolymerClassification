...
def test(args, model_class, test_dataloader, model_weights_path, n_classes=2, arch="trans_pre"):
    ...
    if arch=="incep" or arch=="incep_pre":
        ...
        model = model(in_len=args.input_size, num_class=n_classes).to(device)
    else:
...
if __name__=="__main__":
    ...
    parser.add_argument('--weights', type=str, default='', help='.pt file')

    parser.add_argument('--log_name', type=str, default='exp12_test_W_C4_5000_val', help='Read and Save log file name')
    ...
    parser.add_argument('--arg_file', type=str, default='logFile/exp12_test_W_C4_5000_val/raw/arguments.txt', help='input argument .txt file')
...
# Hyp
    parser.add_argument('--add_base', type=str, default='no', help='do it need to add _baseline to .npy files')
...
# Transformer Hyp
    ...
    if args.model=="all":
        print(f'model in argument file was all, so {m} is used')
        args.model = m
    if args.add_base == "yes":
        args.weights = os.path.join("logFile", args.log_name, args.model)
        if args.save_path == "default":
            args.save_path = os.path.join("logFile",args.log_name, "test", f'test_results_{args.model}_baseline.csv')

    else:
        args.weights = os.path.join("logFile",args.log_name, "weights_"+args.model)

        if args.save_path == "default":
            args.save_path = os.path.join("logFile",args.log_name, "test", f'test_results_{args.model}.csv')
    ...
    for fold_name in os.listdir(f'logFile/{args.log_name}/data_split'):
        ...
        weight_path = os.path.join(args.weights, "best_" + str(args.fold_num) + ".pt")
        if os.path.exists(os.path.join(folder_path, "index_test.txt")):
            dataarrayX_test, dataarrayY_test, data_number_test, data_list = dataset_from_txt(args.data_dir, os.path.join(folder_path, "index_test.txt"))
            args.data_names = data_list
            dataset_test = IRDatasetFromNames(dataarrayX_test, dataarrayY_test, data_number_test)
            ...
            dataarrayX_val, dataarrayY_val, data_number_val, data_list = dataset_from_txt(args.data_dir, os.path.join(folder_path, "index_val.txt"), add_base=args.add_base)
            args.data_names = data_list

            dataset_val = IRDatasetFromNames(dataarrayX_val, dataarrayY_val, data_number_val)    
